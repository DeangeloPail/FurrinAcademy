<html lang="en">
  <head>
   ...
   <!-- Favicon-->
    ...
    <title>Matricula Docentes|FurrinAcademy</title>
 <body class="body"  style="background-image: url('images/cursoprofe.jpg');background-image: no-repeat;
...
<?php 
  ...
  $ListaProfe=$sentenciaSQL->fetchAll(PDO::FETCH_ASSOC);
      ...
      switch($Accion){
        ...
        $sentenciaSQL=$conexionJF->prepare("INSERT INTO curso_profe (codigo_cur_p , Codigo_profesor , Codigo_curso ) 
        VALUES (:CursEstID, :Profesor, :Curso)");
        $sentenciaSQL->bindParam(':CursEstID',$CursEstID);
        $sentenciaSQL->bindParam(':Profesor',$Profesor);
        $sentenciaSQL->bindParam(':Curso',$Curso);
        $sentenciaSQL->execute();
        header("location: curso_profe.php");
        
        break;

        case"Editar";
        $sentenciaSQL=$conexionJF->prepare("UPDATE curso_profe 
        SET Codigo_profesor =:Profesor,Codigo_curso =:Curso WHERE codigo_cur_p =:CursEstID" );
        $sentenciaSQL->bindParam(':CursEstID',$CursEstID);
        $sentenciaSQL->bindParam(':Profesor',$Profesor);
        $sentenciaSQL->bindParam(':Curso',$Curso);
        $sentenciaSQL->execute();
        ...
        case"Eliminar";
        $sentenciaSQL=$conexionJF->prepare("DELETE FROM curso_profe WHERE codigo_cur_p  = :CursEstID" );
        $sentenciaSQL->bindParam(':CursEstID',$CursEstID);
        $sentenciaSQL->execute();
        header("location: curso_profe.php");
        break;
        ...
        case"Seleccionar":
            $sentenciaSQL=$conexionJF->prepare("SELECT * FROM curso_profe WHERE codigo_cur_p  = :CursEstID" );
            $sentenciaSQL->bindParam(':CursEstID',$CursEstID);
            $sentenciaSQL->execute();
            
            $dataCurso=$sentenciaSQL->fetch(PDO::FETCH_LAZY);
    
            $CursEstID=$dataCurso['codigo_cur_p'];
            $Profesor=$dataCurso['Codigo_profesor'];
...
<div class="col-md-5">
    <form id="formulario" method="POST" enctype="multipart/form-data" class="row g-3" >
        ...
        <div class = "form-group">
            ...
            <select class="form-control my-3" name="Profesor" id="Profesor">
                <option selected disabled>Seleccione Profesor...</option>
                <?php foreach($ListaProfe as $Profe){?>
    ...
    <table class="table table-bordered" id="example">
        ...
        <tbody>
            ...
            <tr><td><?php echo $CursoEsto['codigo_cur_p']; ?></td>
                ...
                <form method="post">
                    <input type="hidden" name="CursEstID" id="CursEstID" value="<?php echo $CursoEsto['codigo_cur_p']; ?>">
