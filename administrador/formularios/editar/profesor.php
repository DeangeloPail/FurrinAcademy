...
include('../../sql/vistas/usuarios_p.php');
  $id=$_GET['id'];
  $sql="SELECT * FROM profesores where codigo_profesor='".$id."'";
  $resultadoProfesor=mysqli_query($conexion,$sql);

  $filaProfesor=mysqli_fetch_assoc($resultadoProfesor);
  $codigo=$filaProfesor["codigo_profesor"];
  $nombre=$filaProfesor["Nombre_profe"];
  $direccion=$filaProfesor["Direccion"];
?>
<form id="formulario" action="../../sql/modificar/profesor.php" method="POST" enctype="multipart/form-data" class="row  mx-3 g-3 needs-validation" novalidate >
    ...
    <div class = "form-group">
        ...
        <input type="text" class="form-control mt-3" 
        name="Direccion" id="Direccion"  placeholder="Direccion" value="<?php echo"$direccion";?>">
